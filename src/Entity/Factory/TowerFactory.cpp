...
std::unique_ptr<Tower> TowerFactory::createFromConfigFile(
    ...
    if (!file.is_open()) {
        throw std::runtime_error("TowerFactory: Could not open config file: " + configPath);
    }
    ...
    try {
        ...
        throw std::runtime_error("TowerFactory: JSON parsing error in " + configPath + ": " + e.what());
    }
...
TowerBuilder TowerFactory::builderFromJson(const nlohmann::json& config) {
    ...
    if (config.contains("texture")) {
        ...
        parseTextures(config["texture"], baseTexturePath, turretTexturePath, textureWidth, textureHeight);
    ...
    if (config.contains("stats") && config["stats"].contains("fire_rate")) {
        ...
        if (fireRate > 0) {
            builder.setTimerInterval(1.0f / fireRate); // Convert fire rate to interval
        }
    }
...
std::unique_ptr<TowerStat> TowerFactory::parseStats(const nlohmann::json& statsJson) {
...
void TowerFactory::parseTextures(
    ...
    if (!baseTexturePath.empty()) {
        std::cout << "TowerFactory: Base texture path: " << baseTexturePath << std::endl;
    }
    
    if (!turretTexturePath.empty()) {
        std::cout << "TowerFactory: Turret texture path: " << turretTexturePath << std::endl;
    }
    
    std::cout << "TowerFactory: Texture dimensions: " << width << "x" << height << std::endl;
}

void TowerFactory::validateConfig(const nlohmann::json& config) {
    // Check for required fields
    if (!config.contains("id")) {
        throw std::runtime_error("TowerFactory: Missing required field 'id' in tower configuration");
    }
    ...
    if (config.contains("description") && !config["description"].is_string()) {
        throw std::runtime_error("TowerFactory: Field 'description' must be a string");
    }
    
    if (config.contains("buildable") && !config["buildable"].is_boolean()) {
        throw std::runtime_error("TowerFactory: Field 'buildable' must be a boolean");
    }
    
    if (config.contains("cost") && !config["cost"].is_object()) {
        throw std::runtime_error("TowerFactory: Field 'cost' must be an object");
    }
    
    if (config.contains("stats") && !config["stats"].is_object()) {
        throw std::runtime_error("TowerFactory: Field 'stats' must be an object");
    }
    
    if (config.contains("texture") && !config["texture"].is_object()) {
        throw std::runtime_error("TowerFactory: Field 'texture' must be an object");
    }
}
