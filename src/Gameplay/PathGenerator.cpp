...
PathGenerator::PathGenerator(
    ...
    transitionCost[Height::ShallowWater] = {200, 000, 300, 100, 200, 400, 500, 900};
...
std::vector<sf::Vector2f> PathGenerator::operator()(sf::Vector2i begin,
    ...
    Logger::debug(std::format("Begin A-Star searching from {} {} to {} {}", begin.x, begin.y, end.x, end.y));
    Logger::debug(std::format("The height map size is {} {}", heightMap.size(), heightMap[0].size()));
    while (weight.size()) {
        ...
        // logFile << std::format("Searching at {} {} with g = {}, h = {}\n", current.x, current.y, current.distanceFromStart, current.distanceToEnd);
        // logFile << std::format("Choosing {} {} with f = {}", current.x,
        // current.y, current.distanceFromStart + current.distanceToEnd) <<
        // "\n";
        for (int i = 0; i < 4; i++) {
            Weight nextCell;
