...
void TowerRotationMockScene::createTestTower() {
    ...
    try {
        ...
        std::cout << "Tower stats - Damage: " << testTower->getStat("damage")
                  ...
                  << ", Fire Rate: " << testTower->getStat("fire_rate")
                  << std::endl;
    ...
    // For now, the tower will be created without textures for testing the
    // rotation logic
}
...
void TowerRotationMockScene::setupInfoText() {
    ...
    if (font) {
        ...
        info += "Position: (" +
                std::to_string(static_cast<int>(testTower->getPosition().x)) +
                ", " +
...
void TowerRotationMockScene::updateTowerRotation() {
    ...
    sf::Angle newRotation =
        currentRotation + sf::radians(rotationSpeed * deltaTime);
    ...
    if (font && infoText) {
        ...
        textureStatus +=
            "\nSize: " +
            std::to_string(static_cast<int>(testTower->getTextureWidth())) +
            "x" +
            std::to_string(static_cast<int>(testTower->getTextureHeight()));
        info += textureStatus;
...
void TowerRotationMockScene::draw(sf::RenderTarget& target,
